...
> 状態: **v0.2.1本番稼働中 (2026-08-12デプロイ済み)**。Version ID `18555d03-9148-4e13-b1eb-c1da93f83594`、
> merge commit `7877f30`。
> Neon本番は分析機能着手時まで延期。
...
| mvp        | 架空デモ + 公開データ        | 未接続          | `mvp-slope.mirai-dx-platform.com`     |
| staging    | 承認済み公開データ           | staging         | Access                                |
| production | 承認済みソース               | production      | `terrain-slope.mirai-dx-platform.com` |
...
- **同一オリジン**のため **CORSは不要**、Pagesプロジェクトも不要（Cloudflareリソースは Worker）。
- URLは用途で分離する:
  - MVP/Prototype確認用: `mvp-slope.mirai-dx-platform.com` (`civil-terrain-api-mvp`, `wrangler --env mvp`)
  - 本番用: `terrain-slope.mirai-dx-platform.com` (`civil-terrain-api`)
...
2. 初回 `wrangler deploy` により Worker が作成される（新規リソース作成＝人間承認事項）。
   MVPは `civil-terrain-api-mvp`、本番は `civil-terrain-api` を使い分ける。
   カスタムドメインを使う場合のみ Cloudflare 側でルート/ドメインを設定する
   （DNS変更・custom domain追加は人間決裁）。
3. **secretは現行MVPでは不要**。次の値が必要になった時点で `wrangler secret put <NAME>` により
   人間決裁で登録する（値の登録・変更は人間決裁）。
   - `CF_ACCESS_TEAM_DOMAIN` / `CF_ACCESS_AUD`: Worker側のAccess JWT検証を有効化。
     両方設定した場合のみ有効（片方欠けは無効扱い）。
   - `RATE_LIMIT_PER_MINUTE`: 分あたりのAPI上限（既定 120。通常は変更不要）。
   - `DATABASE_URL`（Neon接続、Hyperdrive推奨）。
...
# 1) MVP/Prototypeデプロイ（SPAビルド → MVP Worker+Assetsを一括アップロード）
pnpm deploy:mvp
#   = pnpm --filter ./apps/web run build
#     && pnpm --filter ./apps/api exec wrangler deploy --env mvp

# 2) 本番デプロイ（SPAビルド → Production Worker+Assetsを一括アップロード）
pnpm deploy:prod
#   = pnpm --filter ./apps/web run build
#     && pnpm --filter ./apps/api exec wrangler deploy
```

- `pnpm deploy:mvp` は `apps/api/wrangler.toml` の `[env.mvp]` を使い、
  `mvp-slope.mirai-dx-platform.com` と Worker `civil-terrain-api-mvp` へdeployする。
  本番URL `terrain-slope.mirai-dx-platform.com` のrouteは変更しない。
- `pnpm deploy:prod` は **SPAを先にビルド**してから `wrangler deploy` を実行する
  (pnpm の組み込み `deploy` コマンドと衝突するためスクリプト名は `deploy:prod`。
  2026-08-12 に改名)
...
- Worker のコード・設定を変えずSPAだけ更新する場合も `pnpm deploy:prod` を使う（assetsは
  Workerデプロイに同梱されるため）。
...
| 確認                                         | 期待                                                                    |
| -------------------------------------------- | ----------------------------------------------------------------------- |
| `GET /` on `mvp-slope.mirai-dx-platform.com` | 200・SPAのindex.htmlが返る                                              |
