...
WorkerはCloudflare標準の `Cf-Access-Jwt-Assertion` ヘッダーを優先し、ローカル検証用に
`Authorization: Bearer` もフォールバックとして受け付けます。`CF_ACCESS_TEAM_DOMAIN` と
`CF_ACCESS_AUD` の片方だけが設定された場合は設定ミスとして保護対象ルートを503で停止します。
...
| Access JWT の Worker 側検証 | `apps/api/src/security/access-auth.ts` — `Cf-Access-Jwt-Assertion` 優先、`Authorization: Bearer` フォールバック。`CF_ACCESS_TEAM_DOMAIN`/`CF_ACCESS_AUD` 両方設定時のみ有効、片方欠けは保護ルート503。RS256署名検証・exp/aud検査・公開鍵キャッシュ。 | 単体テスト (有効/期限切れ/aud不一致/改ざん/鍵不明/証明書障害/ヘッダー抽出) + ルーター統合テスト (401/403/503/公開ルート除外) |
