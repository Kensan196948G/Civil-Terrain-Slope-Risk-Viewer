...
> 📘 **状態:** 設計完了　🚧 **段階:** 実装準備　🎯 **対象:** MVP　📄 **ライセンス:** 未決定

> [!IMPORTANT]
> 現在のリポジトリは **要件定義・詳細設計が完了した実装準備段階** です。アプリケーションコード、実行可能なWeb UI、API、CIはまだありません。
...
現時点では実行コマンドはまだ存在しません。Sprint 0でmonorepo、fixture、CI、データソース台帳を作成した後、次の契約を実コマンドに置き換えます。
